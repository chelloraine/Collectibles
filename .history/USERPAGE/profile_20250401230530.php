...
if (empty($_SESSION['customer_id']) && empty($_SESSION['Admin_id'])) {
    die("Session is not set!");  // Debugging
...
// Get the user's data
$stmt = $conn->prepare("SELECT First_Name, Last_Name, Username, Customer_Email, Contact_Number, Date_Of_Birth FROM Customers WHERE Customer_ID = ? OR Admin_ID = ?");
